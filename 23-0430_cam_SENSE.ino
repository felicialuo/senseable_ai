...
void connectToPeripheral(String pred_result){
  ...
  if (peripheral) {
    ...
    Serial.println(" ");
    BLE.stopScan();
    controlPeripheral(peripheral, pred_result);
  }
}

void controlPeripheral(BLEDevice peripheral, String pred_result) {
  ...
  BLECharacteristic messageCharacteristic = peripheral.characteristic(deviceServiceCharacteristicUuid);
  if (!messageCharacteristic) {
    Serial.println("* Peripheral device does not have message_type characteristic!");
    peripheral.disconnect();
    return;
  } else if (!messageCharacteristic.canWrite()) {
    Serial.println("* Peripheral does not have a writable message_type characteristic!");
    peripheral.disconnect();
    return;
  }
  
  String messageList = formMessage(pred_result);
  int counter = 0;
  // randomSeed(analogRead(0));
  if (peripheral.connected()) {
    
    // message = messageList.substring(0, counter);
    message = String(messageList[counter]);
    // message = random(100);
    ...
    if (oldMessage != message) {  
      oldMessage = message;
      Serial.print("* Message attempt: ");
      Serial.println(message);
      messageCharacteristic.writeValue(message.c_str());
      // messageCharacteristic.writeValue((byte)message);
      Serial.println("* Message sent");
      counter += 1;
    }
    if (counter > (messageList.length())){counter = 0;}
  
  }
  Serial.println("- Peripheral device disconnected!");
}

//prefex "@"" and append "#"" to a message for sending to NANO IOT.
...
/* Includes ---------------------------------------------------------------- */
#include <pen_apple_pineapple_inferencing.h>
...
void loop()
{
    ...
    while(stop_inferencing == false) {
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            pred_result += "\t" + String(result.classification[ix].label) + ": " +
                            String(result.classification[ix].value, 5);
        }

        connectToPeripheral(pred_result);
